import { worldInterface, worldmap } from './model/world';
 ...
 *  @param filePathArr  worldgenoverride的路径
 */
export function readFromFile(map: Map<string, worldmap>): worldInterface {
  let worldObj:any = {};
  worldObj['world'] = {};
  ...
  map.forEach((wm, flag) => {
    ...
    IDValues.forEach(function (value, key) {
      ...
      worldObj['world'][flag]["data"][key]['option'] = worldItemsJson['worldItems'][key] == undefined ? { default: "默认" } : (worldItemsJson['worldItems'][key]["option"]);
 ...
 */
function readFromFile_IDValue(filePath: string): Map<string, string> {

  // 解析lua字符串
  var ast:any = parser.parse(fs.readFileSync(filePath).toString());
  let worldItems: any[] = ast['body'][0]['arguments'][0]['fields'][2]['value']['fields'];

  // 设置返回值
  let IDs_and_Values: Map<string, string> = new Map<string, string>();

  // 遍历worldItems数组 然后返回
  worldItems.forEach(element => {

    let ID: string = element['key']['name'];       /* ID值 */
    let value: string = element['value']['value']; /* value值 */
 ...
 * @param worldi 世界对象
 */
export function writeFile(worldi: worldInterface): void {
    ...
    if (worldi.world.hasOwnProperty(key)) {
      ...
      for (const id in data) {
        if (data.hasOwnProperty(id)) {
          ...
          // 拼接
          pinjie(whichType, id, value, desc, option);
      ...
      function pinjie(whichType: string, id: string, value: string, desc: string, option: string) {
        if (whichType == "World") {
